...
class AnomalyAE(nn.Module):
    def __init__(self, input_dim=81, latent_dim=32):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Linear(128, 64), nn.BatchNorm1d(64), nn.LeakyReLU(0.1), nn.Linear(64, latent_dim)
        )
        self.decoder = nn.Sequential(
            ...
            nn.Linear(256, 512), nn.BatchNorm1d(512), nn.LeakyReLU(0.1), nn.Dropout(0.1), nn.Linear(512, input_dim)
        )
...
class AETrainer:
    def __init__(self, input_dim=81, encoding_dim=32, lr=1e-3):
        ...
        self.criterion = nn.MSELoss(); self.optimizer = optim.Adam(self.model.parameters(), lr=lr); self.known_threshold = None
    def train_on_known_data(self, X_benign, epochs=50, batch_size=1024, verbose=True):
        if verbose: print(f"Training Deep AE on {len(X_benign)} samples...")
        self.model.train(); tensor = torch.FloatTensor(X_benign).to(self.device)
        ...
        for epoch in range(epochs):
            for batch in loader:
                self.optimizer.zero_grad(); loss = self.criterion(self.model(batch[0]), batch[0]); loss.backward(); self.optimizer.step()
        self.model.eval(); errors = self.get_reconstruction_errors(X_benign)
    ...
    def get_reconstruction_errors(self, data):
        self.model.eval()
        with torch.no_grad():
            tensor = torch.FloatTensor(data).to(self.device); errors = []
            for i in range(0, len(tensor), 2048):
                batch = tensor[i:i+2048]; errors.append(torch.mean((batch - self.model(batch))**2, dim=1).cpu().numpy())
...
class IncrementalOCSVM:
    def __init__(self, nu=0.15, random_state=42):
        self.feature_map = Nystroem(gamma=0.1, random_state=random_state, n_components=300)
        self.model = SGDOneClassSVM(nu=nu, random_state=random_state, shuffle=True); self.is_fitted = False
    def train(self, X): self.model.fit(self.feature_map.fit_transform(X)); self.is_fitted = True
    def partial_fit(self, X): self.model.partial_fit(self.feature_map.transform(X)) if self.is_fitted else self.train(X)
...
class OpenSetXGBoost:
    def __init__(self, confidence_threshold=0.7, max_classes_buffer=20):
        self.confidence_threshold = confidence_threshold; self.model = None; self.label_encoder = {}; self.reverse_encoder = {}; self.max_classes = max_classes_buffer
    def _update_encoder(self, y):
        cur = set(self.label_encoder.keys()); new = set(np.unique(y)) - cur; idx = len(self.label_encoder)
    ...
    def train(self, X, y, is_incremental=False):
        ...
        X_train, X_val, y_train, y_val, w_train, w_val = train_test_split(X, y_enc, sample_weights, test_size=0.1, random_state=42, stratify=y_enc)
        if not is_incremental:
            self.model = xgb.XGBClassifier(n_estimators=300, max_depth=6, learning_rate=0.1, objective='multi:softprob', num_class=self.max_classes, n_jobs=-1, random_state=42)
            self.model.fit(X_train, y_train, sample_weight=w_train, eval_set=[(X_val, y_val)], verbose=False)
        else:
            self.model.set_params(n_estimators=self.model.get_booster().num_boosted_rounds() + 50)
    ...
    def predict_with_confidence(self, X):
        proba = self.model.predict_proba(X); valid = list(self.reverse_encoder.keys()); valid_proba = proba[:, valid]
        idx = np.argmax(valid_proba, axis=1)
        return np.array([self.reverse_encoder[valid[i]] for i in idx]), np.max(valid_proba, axis=1)
    def safe_incremental_retrain(self, X_old, y_old, X_new, y_new): self.train(np.vstack([X_old, X_new]), np.hstack([y_old, y_new]), is_incremental=True)
