...
def session1_workflow():
    ...
    # 1. Init Loader & Load Global Scaler
    loader = SessionDataLoader()
    loader.load_scaler(GLOBAL_SCALER_PATH)
    ...
    metrics_data['XGBoost']['Pre'] = evaluate_supervised_with_unknown(
        y_train, xgb_pre, xgb_conf, 
        atk_thres=0.7, ben_thres=0.7, 
        session_name="Case1_PreIL", save_dir=save_dir, target_unknown=3
    )
    
    evaluate_gray_zone(y_train, xgb_pre, xgb_conf, details['ae_pred'], details['ocsvm_pred'], 0.7, 0.95, "Case1_PreIL", save_dir)
    
    ae_f1, oc_f1 = evaluate_unsupervised_detailed(y_train, details['ae_pred'], details['ocsvm_pred'], "Case1_PreIL", save_dir, return_f1=True)
    ...
    metrics_data['Pipeline']['Post'] = evaluate_final_pipeline(y_test, final_preds, "Case1_PostIL", save_dir, return_f1=True)
    
    xgb_post, _ = pipeline.xgb.predict_with_confidence(X_test)
    metrics_data['XGBoost']['Post'] = evaluate_supervised_model(y_test, xgb_post, "Case1_PostIL", save_dir, "XGBoost", return_f1=True)
    
    ae_f1_p, oc_f1_p = evaluate_unsupervised_detailed(y_test, details_test['ae_pred'], details_test['ocsvm_pred'], "Case1_PostIL", save_dir, return_f1=True)
    metrics_data['AE']['Post'] = ae_f1_p; metrics_data['OCSVM']['Post'] = oc_f1_p
    
    plot_comparison_chart(metrics_data, "Case 1", f"{save_dir}/comparison_chart_sess1.png")
